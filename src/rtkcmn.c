...
*-----------------------------------------------------------------------------*/
extern int satno(int sys, int prn)												//将卫星系统和伪随机噪声编号，转为卫星编号
{
    if (prn<=0) return 0;
    switch (sys) {
        case SYS_GPS:
...
*-----------------------------------------------------------------------------*/
extern int satsys(int sat, int *prn)											//卫星编号转卫星系统
{
    int sys=SYS_NONE;
...
*-----------------------------------------------------------------------------*/
extern int satid2no(const char *id)												//卫星ID转卫星编号
{
    int sys,prn;
    char code;
...
* return : none
*-----------------------------------------------------------------------------*/
extern void satno2id(int sat, char *id)											//卫星编号转卫星ID
{
    int prn;
...
* return : none
*-----------------------------------------------------------------------------*/
extern void setcodepri(int sys, int freq, const char *pri)						//设置代码优先级（这是针对多个卫星系统的）
{
...
* return : || a ||
*-----------------------------------------------------------------------------*/
extern double norm(const double *a, int n)										//欧几里得范数，说白了，就是同一个向量点乘后开方，求向量长度
{
...
* return : none
*-----------------------------------------------------------------------------*/
extern void cross3(const double *a, const double *b, double *c)					//三维向量外积（叉积）(x1,y1,z1)×(x2,y2,z2)=(y1z2-y2z1,z1x2-z2x1,x1y2-x2y1)；
{																				//新向量长度|z|=|x||y|*sin<x,y>，几何意义就是求垂直于xy方向上的向
    c[0]=a[1]*b[2]-a[2]*b[1];
    c[1]=a[2]*b[0]-a[0]*b[2];
    c[2]=a[0]*b[1]-a[1]*b[0];
}
...
*-----------------------------------------------------------------------------*/
extern int normv3(const double *a, double *b)									//三维向量归一化；就是将一个向量，变成长度为1，方向不变的向量。
{
    double r;
                  ...
                  const double *R, int n, int m)
{
	...
	trace(1, "x_=\n"); tracemat(1, x_, 1, k, 8, 4);
	trace(1, "P_=\n"); tracemat(1, P_, k, k, 8, 4);
...
extern int filter_Cvv(double* x, double* P, const double* H, const double* v,
	const double* R, int n, int m)
{
	double* x_, * xp_, * P_, * Pp_, * H_;
	int i, j, k, info, * ix;
	...
	//matcpy(Cvv, R, m, m);
	//if ((info = matinv(R, m))) {
	//	matmul("NN", n, m, n, 1.0, R, H, 0.0, F);       /* Cvv=R-H(H^T*R^-1*H)^-1*H^T */
	//}
...
*          matirix stored by column-major order (fortran convention)
*-----------------------------------------------------------------------------*/
extern int smoother(const double *xf, const double *Qf, const double *xb,		//卡尔曼平滑 
