                    ...
                    rtk_t *rtk, int mode)
{
    ...
    while ((nobs=inputobs(obs,rtk->sol.stat,popt))>=0) {									//input obs data, navigation messages and sbas correction 
        
        /* exclude satellites */															//exclude satellites
        for (i=n=0;i<nobs;i++) {
...
#endif
         if (!rtkpos(rtk,obs,n,&navs)) {													//start rtkpos
                  ...
                  const prcopt_t *opt)
{
    ...
    for (iobs=0;(m=nextobsf(obs,&iobs,rcv))>0;iobs+=m) {
        ...
        if (!pntpos(data,j,nav,opt,&sol,NULL,NULL,msg)) continue;			//standard position
                   ...
                   char **infile, const int *index, int n, char *outfile)
{
    ...
    if (popt_.mode==PMODE_SINGLE||popt_.soltype==0) {							//start process positioning
                     ...
                     const char *rov, const char *base)
{
	...
	//printf("execses_b: n=%d outfile=%s\n", n, outfile);
                   ...
                   const char *rov, const char *base)
{
	...
	//printf("postpos : ti=%.0f tu=%.0f n=%d outfile=%s\n", ti, tu, n, outfile);
