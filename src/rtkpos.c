...
/* adjust initialize state and covariance -------------------------------------------*/
static void initx_adj(rtk_t* rtk, double* xp, double xi, double var, int i)
{
	int j;
	*(xp+i) = xi;
	for (j = 0; j < rtk->nx; j++) {
...
static void udpos(rtk_t *rtk, double tt)
{
    ...
    /* include accel terms if filter is converged收敛 */
                           ...
                           const nav_t *nav)
{
    ...
    g0=rtk->ssat[sat-1].gf; rtk->ssat[sat-1].gf=g1;//频率单差之差L1-L2，初始为0
        
    if (g0!=0.0&&fabs(g1-g0)>rtk->opt.thresslip) {
        
        rtk->ssat[sat-1].slip[0]|=1;
        rtk->ssat[sat-1].slip[1]|=1;
		//printf("GF thresslip is %f\n", rtk->opt.thresslip);//0.05m
		printf("slip detected GF2_jump (sat=%2d GF_L1_L2=%.3f %.3f fabs(g1-g0)=%.3f)\n", sat, g0, g1, fabs(g1 - g0));
                           ...
                           const nav_t *nav)
{
    ...
    if (g0!=0.0&&fabs(g1-g0)>rtk->opt.thresslip) {
		...
		printf("slip detected GF5_jump (sat=%2d GF_L1_L5=%.3f %.3f fabs(g1-g0)=%.3f)\n", sat, g0, g1, fabs(g1 - g0));
...
{
	const double* lam = nav->lam[obs->sat - 1];//波长
	double l1, l2, p1, p2;
	...
	l1 = sdobs(obs, iu, ir, 0);   //载波
	l2 = sdobs(obs, iu, ir, i);
	p1 = sdobs(obs, iu, ir, 0 + NFREQ);//伪距
...
static void detslp_mw_n(rtk_t* rtk, const obsd_t* obs, int iu, int ir,
	int sat, const nav_t* nav)
{
	double w0, w1, elev, el, dtmp;
	register int j;
	...
	//printf("detslip_mw: sat=%2d mw0=%8.3f mw1=%8.3f fabs(w1 - w0)=%8.3f\n", sat, w0, w1, fabs(w1 - w0));

	//if (sample > 0.0) {
	//	if (sample <= 1.0)        ThresMW = 2.5;
	//	else if (sample <= 20.0)  ThresMW = (2.5) / (20.0 - 0.0) * sample + 2.5;
	//	else if (sample <= 60.0)  ThresMW = 5.0;
	//	else                      ThresMW = 7.5;
	//}
	...
	//if (el >= rad_20) THRES_MW_JUMP = ThresMW;
	//else	 THRES_MW_JUMP = -ThresMW * 0.1 * dtmp + 3 * ThresMW;

	//printf("sample=%f, ThresMW=%f, el=%f, THRES_MW_JUMP=%f\n", sample, ThresMW, dtmp, THRES_MW_JUMP);

	//if (w0 != 0.0 && fabs(w1 - w0) > THRES_MW_JUMP) {
	if (w0 != 0.0 && fabs(w1 - w0) > THRES_MW_JUMP) {
		printf("detslip_mw: slip detected sat=%2d mw0=%8.3f mw1=%8.3f fabs(w1 - w0)=%8.3f\n\n",sat, w0, w1, fabs(w1 - w0));
		errmsg(rtk, "detslip_mw: slip detected sat=%2d mw=%8.3f->%8.3f\n",
			sat, w0, w1);
		for (j = 0; j < rtk->opt.nf; j++) rtk->ssat[sat - 1].slip[j] |= 1;    //标记周跳
	}
}

/*PPP Melbourne-Wubbena linear combination --------------------------------------*/
static double mwmeas_ppp(const obsd_t* obs, const nav_t* nav)
{
...
/*PPP detect slip by Melbourne-Wubbena linear combination jump ------------------*/
static void detslp_mw_ppp(rtk_t* rtk, const obsd_t* obs, int n, const nav_t* nav)
{
	double w0, w1, elev, el, dtmp;
	int i, j;
	...
	for (i = 0; i < n && i < MAXOBS; i++) {
		if ((w1 = mwmeas_ppp(obs + i, nav)) == 0.0) continue;
		...
		elev = rtk->ssat[obs[i].sat - 1].azel[1];
		el = elev;
	...
	const int* iu, const int* ir, int ns, const nav_t* nav)
{
	...
	for (i = 0; i < ns; i++) {
		...
		/* detect cycle slip by LLI 通过观测值的失锁标识符来检测周跳*/
		for (f = 0; f < rtk->opt.nf; f++) rtk->ssat[sat[i] - 1].slip[f] &= 0xFC;
		detslp_ll(rtk, obs, iu[i], 1);												//流动站
		detslp_ll(rtk, obs, ir[i], 2);												//基准站

		/* detect cycle slip by geometry-free phase jump 电离层残差法进行周跳探测，没有周跳时它随时间变化缓慢*/
	...
	for (f = 0; f < nf; f++) {
		...
		for (i = 0; i < ns; i++) {
			...
			slip = rtk->ssat[sat[i] - 1].slip[f];											////|=1 标记周跳
		...
		for (i = 0; i < ns; i++) {
			...
			initx(rtk, (bias[i] - rtk->com_bias) / lami, SQR(rtk->opt.std[0]), IB(sat[i], f, &rtk->opt));  //重新初始化
...
/* temporal update of phase biases -------------------------------------------*/
static void udbias_MW(rtk_t *rtk, double tt, const obsd_t *obs, const int *sat,
                   const int *iu, const int *ir, int ns, const nav_t *nav)
{
    double cp,pr,cp1,cp2,pr1,pr2,*bias,offset,lami,lam1,lam2,C1,C2;
    int i,j,f,slip,rejc,reset,nf=NF(&rtk->opt),sysi;
    
    trace(3,"udbias-MW : tt=%.3f ns=%d\n",tt,ns);
    ...
    for (i=0;i<ns;i++) {
		...
		/* detect cycle slip by MW-measurement */
		//detslp_mw_n(rtk, obs, iu[i], ir[i], sat[i], nav);
		//detslp_mw(rtk, obs, n, nav);
		detslp_mw_ppp(rtk, obs, ns, nav);
...
static void udstate(rtk_t *rtk, const obsd_t *obs, const int *sat,
                    const int *iu, const int *ir, int ns, const nav_t *nav)
{
    double tt=rtk->tt,bl,dr[3];				//tt: time difference between current and previous (s) 
    ...
    if (rtk->opt.mode>PMODE_DGPS) {
		...
		udbias_MW(rtk, tt, obs, sat, iu, ir, ns, nav);
    }
}
                 ...
                 int index, double *y, double *e, double *azel)
{
    ...
    for (i=0;i<n;i++) {                                                                           //n为观测到卫星数，判断是否要隔离
        /* compute geometric-range and azimuth/elevation angle */
        if ((r=geodist(rs+i*6,rr_,e+i*3))<=0.0) continue;
        if (satazel(pos,e+i*3,azel+i*2)<opt->elmin) continue;
        
        /* excluded satellite? */
        if (satexclude(obs[i].sat,var[i],svh[i],opt)) continue;                                    //sat exclude
...
static int ddres_adj(rtk_t* rtk, const prcopt_t* opt, const nav_t* nav, const obsd_t* obs, double dt, const double* x,
	const double* P, const int* sat, double* y, double* e,
	double* azel, const int* iu, const int* ir, int ns, double* v,
	double* H, double* R, int* vflg)
{
...
/* resolve integer ambiguity by LAMBDA divided---------------------------------------*/					//解模糊度
static int resamb_LAMBDA_div(rtk_t *rtk, double *bias, double *xa,int gps,int glo,int sbs)
{
    prcopt_t *opt=&rtk->opt;
    int i,j,ny,nb,info,nx=rtk->nx, f, nf = NF(&rtk->opt), k, na=rtk->na, nofix, ng=0, nr=0, nc=0;
    ...
    D=zeros(nx,nx);																					//构造D，每个系统，每个频率一个D，双系统双频4个D
    ...
    matmul("TN",ny, 1,nx,1.0,D ,rtk->x,0.0,y );   /* y=D'*x */										//D*X双差模糊度
    matmul("TN",ny,nx,nx,1.0,D ,rtk->P,0.0,DP);   /* DP=D'*P */										//DPD双差模糊度方差
	...
	//					rtk->ssat[j - k].azel[1] >= rtk->opt.elmaskar && !nofix) {
	//					
	//					/* Count the number of satellites in each system */
	//					if (test_sys(rtk->ssat[j - k].sys, 0)) {
	//						ngf1++;
	//					}
	//					else if (test_sys(rtk->ssat[j - k].sys, 1)) {
	//						nr++;
	//					}
	//					else if (test_sys(rtk->ssat[j - k].sys, 3)) {
	//						nc++;
	//					}
	//				}
	//			}
	...
	//if (ng != 0) {
	//	lambda(ng, 2, y + na + ng, Qbg, b, s);
	//}
	//if (nr != 0) {
	//	lambda(nr, 2, y + na + ng + nr, Qbr, b, s);
	//}
	//if (nc != 0) {
	//	lambda(nc, 2, y + na + ng + nr + nc, Qbc, b, s);
	//}

    if (!(info=lambda(nb,2,y+na,Qb,b,s))) {															//y浮点解，Qb方差，扔进LAMBDA去
...
/* resolve integer ambiguity by LAMBDA ---------------------------------------*/					//解模糊度
...
/* validation of solution 添加绝对检查----------------------------------------------------*/
static int valpos_code(rtk_t* rtk, const double* v, const double* R, const int* vflg,                //残差绝对相对检查
	int nv, const obsd_t* obs, prcopt_t* opt)
{
#if 0
	prcopt_t * opt = &rtk->opt;
	double vv = 0.0;
#endif
	double thresP1 = 3.0, thresP2 = 2.0;
	double factP = thresP2 * thresP2;
	int i, stat = 0, sat1, sat2, type, freq;
	char* stype;
	double el;

	trace(3, "valpos  : nv=%d thresP1=%.1f\n thresP2=%.1f\n", nv, thresP1, thresP2);
	...
	//		if (v[i] * v[i] > factP * R[i + i * nv] && v[i] * v[i] > (factP * R[i + i * nv] / el)) {
	//			opt->exsats[obs[i].sat - 1] = 1; 
	//			return 0;
	//		}
	//		else {
	//			return 1;
...
/* validation of solution 添加绝对检查----------------------------------------------------*/
static int valpos_phase(rtk_t* rtk, const double* v, const double* R, const int* vflg,                //残差绝对相对检查
	int nv, const obsd_t* obs)
{
#if 0
	prcopt_t * opt = &rtk->opt;
	double vv = 0.0;
#endif
	double thresL1 = 0.03, thresL2 = 2.0;
	double factL = thresL2 * thresL2;
	...
	for (i = 0; i < nv; i++) {
		...
		if (stype == "L") {
			el = rtk->ssat[obs[i].sat - 1].azel[1];
			if (v[i] * v[i] > factL * R[i + i * nv] && v[i] * v[i] > (factL * R[i + i * nv] / el)) {
					initx(rtk, 0.0, 0.0, IB(i, freq, &rtk->opt));
...
static int valpos(rtk_t *rtk, const double *v, const double *R, const int *vflg,                //相对检查，添加绝对检查
                  int nv, double thres)
{
#if 0
    prcopt_t *opt=&rtk->opt;
 ...
 */
static int relpos(rtk_t* rtk, const obsd_t* obs, int nu, int nr,											//主函数，相对定位
	const nav_t* nav)
{
	prcopt_t* opt = &rtk->opt;
	...
	if (!zdres(1, obs + nu, nr, rs + nu * 6, dts + nu * 2, var + nu, svh + nu, nav, rtk->rb, opt, 1,		//基准站做差，base做一个非差残差，SPP方程
	...
	udstate(rtk, obs, sat, iu, ir, ns, nav);																//udbias卡尔曼滤波时间更新
	...
	niter = opt->niter + (opt->mode == PMODE_MOVEB && opt->baseline[0] > 0.0 ? 2 : 0);								//movingbase 加两个迭代
	...
	for (i=0;i<niter;i++) {																			
		/* calculate zero diff residuals [range - measured pseudorange] for rover (phase and code)
			output is in y[0:nu-1], only shared input with base is nav
				...
				azel = [az, el] to sats                                   */
		trace(3, "rover:\n");
		if (!zdres(0, obs, nu, rs, dts, var, svh, nav, xp, opt, 0, y, e, azel)) {                                  //流动站，重新展开滤波
		...
		if ((info = filter(xp, Pp, H, v, R, rtk->nx, nv))) {												    //卡尔曼滤波量测更新
			errmsg(rtk, "filter error (info=%d)\n", info);
			stat = SOLQ_NONE;
			break;
		}
	...
	/* calc zero diff residuals again after kalman filter update */                                 //验后残差
	if (stat != SOLQ_NONE && zdres(0, obs, nu, rs, dts, var, svh, nav, xp, opt, 0, y, e, azel)) {
		...
		if (valpos(rtk, v, R, vflg, nv, 4.0)) {															//验后残差检验  valpos
	...
	if (stat != SOLQ_NONE && rtk->opt.modear == ARMODE_WLNL) {											//模糊度固定
...
/* relpos()relative positioning adjust------------------------------------------------------
 *  args:  rtk      IO      gps solution structure
           obs      I       satellite observations
           nu       I       # of user observations (rover)
           nr       I       # of ref observations  (base)
           nav      I       satellite navigation data
 */
static int relpos_adj(rtk_t *rtk, const obsd_t *obs, int nu, int nr,											//主函数，相对定位
                  const nav_t *nav)
{
	//prcopt_t* opt = &rtk->opt;
	prcopt_t opt=rtk->opt;
    ...
    int nf=opt.ionoopt==IONOOPT_IFLC?1:opt.nf;
	double MW_EL;
	int sat1, sat2, type, freq=0, vLfreq = 0, v_Lfreq = 0;
	int vPsat2, v_Psat2, vLsat2, v_Lsat2 = 0;
	double vPmax = 0.0, v_Pmax=0.0, vLmax=0.0, v_Lmax = 0.0;
	double dvPmax, dv_Pmax, dvLmax, dv_Lmax;
	double thresP1 = 3.0, thresP2 = 2.0, thresL1 = 0.3, thresL2 = 15.0;////
	double factP1= thresP1 * thresP1, factP2 = thresP2 * thresP2, factL1 = thresL1 * thresL1,factL2 = thresL2 * thresL2;
	double el=0.0, el2=0.0;
	...
	printf("relpos_adj  : nx=%d nu=%d nr=%d\n", rtk->nx, nu, nr);
    trace(3,"relpos  : nx=%d nu=%d nr=%d\n",rtk->nx,nu,nr);
    ...
    //for (i=0;i<niter;i++) {																			
	while (1) {																							//更改使一直滤波，判断条件为检查结果满足精度为止
        /* calculate zero diff residuals [range - measured pseudorange] for rover (phase and code)
            output is in y[0:nu-1], only shared input with base is nav 
                obs  = sat observations
                nu   = # of sats
        ...
        /* calculate double-differenced residuals and create state matrix from sat angles 
                ...
                O R = double diff measurement error covariances
                O vflg = list of sats used for dd  */
        if ((nv=ddres_adj(rtk, &opt, nav,obs,dt,xp,Pp,sat,y,e,azel,iu,ir,ns,v,H,R,vflg))<1) {
            errmsg(rtk,"no double-differenced residual\n");
            stat=SOLQ_NONE;
            break;
        }
        ...
        //trace(4,"x(%d)=",i+1); tracemat(4,xp,1,NR(&opt),13,4);
		//trace(1, "x=\n"); tracemat(1, xp, 1, NR(&opt), 13, 4);
		//trace(1, "P=\n"); tracemat(1, Pp, rtk->nx, rtk->nx, 13, 4);



		//TODO Calculate residual V and standardized residual V_

		/* calc zero diff residuals again after kalman filter update */                                 //验后残差
	if (stat != SOLQ_NONE && zdres(0, obs, nu, rs, dts, var, svh, nav, xp, &opt, 0, y, e, azel)) {

		/* calc double diff residuals again after kalman filter update for float solution */
		nv = ddres_adj(rtk, &opt, nav, obs, dt, xp, Pp, sat, y, e, azel, iu, ir, ns, v, NULL, R, vflg);

		////////////////////////////////////////////////////////////////////////////////////
		//计算后验方差协方差阵
		double* Cvv = mat(nv, nv);
		double* F = mat(nv, rtk->nx);
		double* F2 = mat(rtk->nx, rtk->nx);
		double* F3 = mat(rtk->nx, nv);

		matcpy(Cvv, R, nv, nv);
		if ((info = matinv(R, nv))) {
			matmul("NT", nv, rtk->nx, nv, 1.0, R, H, 0.0, F);       /* Cvv=R-H'(H*R^-1*H')^-1*H */
			matmul("NN", rtk->nx, nv, rtk->nx, 1.0, H, F, 0.0, F2);
			if ((info = matinv(F2, rtk->nx))) {
				matmul("NN", rtk->nx, nv, rtk->nx, 1.0, F2, H, 0.0, F3);
				matmul("TN", nv, nv, rtk->nx, -1.0, H, F3, 1.0, Cvv);
			}
		}

		free(F), free(F2), free(F3);
		////////////////////////////////////////////////////////////////////////////////////
		...
		if (nv < 8) {
			...
			for (i = 0; i < nv; i++) {
				...
				printf("i=%2d, sat1=%3d, sat2=%3d, type=%d, freq=%d, v=% 3.6f \n", i, sat1, sat2, type, freq, v[i]);
			}
			...
			for (i = 0; i < nv; i++) {
				if (((vflg[i] >> 4) & 0xF) == 1) {
					if (fabs(v[i]) > fabs(vPmax)) {
						vPmax = v[i];
						vPsat2 = (vflg[i] >> 8) & 0xFF;
						el = rtk->ssat[vPsat2 - 1].azel[1];
					}
					//if (fabs(v[i] * v[i] / R[i + i * nv]) > fabs(v_Pmax)) {
					if (fabs(v[i] * v[i] / Cvv[i + i * nv]) > fabs(v_Pmax)) {
						v_Pmax = v[i] * v[i] / Cvv[i + i * nv];
						v_Psat2 = (vflg[i] >> 8) & 0xFF;
					}
				}
			}

			trace(1, "vPmax=%f, vPsat2=%d, v_Pmax=%f, v_Psat2=%d\n", vPmax, vPsat2, v_Pmax, v_Psat2);
			//printf("vPmax=%f, vPsat2=%d, v_Pmax=%f, v_Psat2=%d\n", vPmax, vPsat2, v_Pmax, v_Psat2);

			//若不满足伪距条件，则排除卫星
			dvPmax = vPmax * vPmax;
			dv_Pmax = v_Pmax * v_Pmax;
			el2 = el * el;
			if ((dv_Pmax > factP2) || (dvPmax > (factP1 / el2))) {
				if (dv_Pmax > factP2) {
					opt.exsats[v_Psat2 - 1] = 1; // excluded satellites (1:excluded,2:included) 
					trace(1,"**************************************************************************************\n");
					trace(1,"*********************** excluded satellites %03d **************************************\n", v_Psat2);
					trace(1,"**************************************************************************************\n");
					free(Cvv);
					//trace(1, "x=\n"); tracemat(1, xp, 1, NR(&opt), 13, 4);
					//trace(1, "P=\n"); tracemat(1, rtk->P, rtk->nx, rtk->nx, 8, 4);
					//printf("**************************************************************************************\n");
					//printf("*********************** excluded satellites %03d **************************************\n", v_Psat2);
					//printf("**************************************************************************************\n");
					continue;
				}
				if (dvPmax > (factP1 / el2)) {
					opt.exsats[vPsat2 - 1] = 1;
					trace(1,"**************************************************************************************\n");
					trace(1,"*********************** excluded satellites %03d **************************************\n", vPsat2);
					trace(1,"**************************************************************************************\n");
					free(Cvv);
			...
			//}
				...
				for (i = 0; i < nv; i++) {
					if (((vflg[i] >> 4) & 0xF) == 0) {
						if (fabs(v[i]) > fabs(vLmax)) {
							vLmax = v[i];
							vLsat2 = (vflg[i] >> 8) & 0xFF;
							el = rtk->ssat[vLsat2 - 1].azel[1];
							vLfreq = vflg[i] & 0xF;
						}
						if (fabs(v[i] * v[i] / Cvv[i + i * nv]) > fabs(v_Lmax)) {
							v_Lmax = v[i] * v[i] / Cvv[i + i * nv];
							v_Lsat2 = (vflg[i] >> 8) & 0xFF;
							v_Lfreq = vflg[i] & 0xF;
						}
					}
				}

				trace(1,"vLmax=%f, vLsat2=%d, v_Lmax=%f, v_Lsat2=%d\n", vLmax, vLsat2, v_Lmax, v_Lsat2);
				//printf("vLmax=%f, vLsat2=%d, v_Lmax=%f, v_Lsat2=%d\n", vLmax, vLsat2, v_Lmax, v_Lsat2);

				//若不满足相位条件，则重新初始化
				dvLmax = vLmax * vLmax;
				dv_Lmax = v_Lmax * v_Lmax;
				el2 = el * el;
				if ((dv_Lmax > factL2) || (dvLmax > (factL1 / el2))) {
					//trace(1, "x=\n"); tracemat(1, xp, 1, NR(&opt), 13, 4);
					//trace(1, "P=\n"); tracemat(1, rtk->P, rtk->nx, rtk->nx, 8, 4);

					if (dv_Lmax > factL2) {
						//initx(rtk, 0.0, 0.0, IB(v_Lsat2, v_Lfreq, &opt));
						initx_adj(rtk, xp, 0.0, 900.0, IB(v_Lsat2, v_Lfreq, &opt));
						trace(1,"**************************************************************************************\n");
						trace(1,"*********************** initialize covariance sat %03d f %d ****************************\n", v_Lsat2, v_Lfreq);
						trace(1,"**************************************************************************************\n");
						free(Cvv);
						//printf("**************************************************************************************\n");
						//printf("*********************** initialize covariance sat %03d f %d ****************************\n", v_Lsat2, v_Lfreq);
						//printf("**************************************************************************************\n");
						//trace(1, "x=\n"); tracemat(1, xp, 1, NR(&opt), 13, 4);
						//trace(1, "P=\n"); tracemat(1, rtk->P, rtk->nx, rtk->nx, 8, 4);
						continue;
					}
					if (dvLmax > (factL1 / el2)) {
						//initx(rtk, 0.0, 0.0, IB(vLsat2, vLfreq, &opt));
						initx_adj(rtk, xp, 0.0, 900.0, IB(v_Lsat2, v_Lfreq, &opt));
						trace(1,"**************************************************************************************\n");
						trace(1,"*********************** initialize covariance sat %03d f %d ****************************\n", vLsat2, vLfreq);
						trace(1,"**************************************************************************************\n");
		...
		////////////////////////////////////////////////////////////////////////////////////
		/* validation of float solution, always returns 1, msg to trace file if large residual */
		//if (valpos_code(rtk, v, R, vflg, nv, obs, opt)) {											//验后残差检验  valpos
		//	if (valpos_phase(rtk, v, R, vflg, nv, obs)) {
		//		break;
		//	}
		//	else continue;
		//}
		...
		//TODO Posterior code residual check
		  //if yes -> { Posterior phase residual check
		  //					if yes -> {output}
	      //						else -> {	}    
		  //			}
		  //   else -> {	}
		...
		//更改验后残差检验
		//挨个检查valpos   不行删卫星  rtk->ssat[sat[j]-1].vsat[frq]=0 (模糊度固定时不用) ||  自己加一个量 ||
		//*** opt->exsats[sat-1]=1   excluded satellite(satexclude())

		//载波检验valpos   有问题重新初始化initx(rtk,0,0,IB(卫星，频率，opt))
		//输出 x[index] = 0.0, p[index]row col ->0.0,主对角元素900，其余是0 ，证明初始化

		//所有检验全部通过，break，否则continue
		////////////////////////////////////////////
